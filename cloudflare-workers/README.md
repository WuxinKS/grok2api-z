...
KV Namespace 建议命名为：`grok2api-z-cache`

如果你使用 GitHub Actions（推荐），会在部署前自动：
- 创建（或复用）D1 数据库：`grok2api-z`
- 创建（或复用）KV namespace：`grok2api-z-cache`
...
仓库已包含工作流：`当前仓库后续可自行添加 cloudflare-workers 专用工作流`，在 `main` 分支 push 时会自动：

1. `npm ci` + `npm run typecheck`
2. 自动创建/复用 D1 + KV，并生成 `wrangler.toml`
3. `wrangler d1 migrations apply DB --remote --config wrangler.toml`
4. `wrangler deploy`
...
1. Workers & Pages → 选择 `grok2api-z-workers` 这个 Worker
